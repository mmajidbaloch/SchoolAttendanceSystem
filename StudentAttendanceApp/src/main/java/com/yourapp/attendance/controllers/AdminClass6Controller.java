...
public class AdminClass6Controller {

    @FXML private Label teacherNameWithClassId106;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC106;
    @FXML private Button updateStudentAttendenceRecordC106;
    @FXML private TableColumn<Student, String> takenByTeacherClass106;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC106;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC106;


    @FXML private TableView<?> fetchStudentsTable;
    @FXML private TableView<Student> class6TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs106;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs106;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs106;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs106;

    private ObservableList<Student> studentList106 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();


    private final String CLASS_ID = "106";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId106.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId106.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId106.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass6() {
    ...
    private void loadClass6Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '106'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs106.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs106.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs106.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs106.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class6TableView.setItems(studentList106);
            takenByTeacherClass106.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC106.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC106.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC106.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC106.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId106.setText("Assigned Teacher: " + selectedUsername);
