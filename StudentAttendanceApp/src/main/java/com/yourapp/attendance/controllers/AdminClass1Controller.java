...
public class AdminClass1Controller {

    @FXML private Label teacherNameWithClassId101;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC101;
    @FXML private Button updateStudentAttendenceRecordC101;
    @FXML private TableColumn<Student, String> takenByTeacherClass101;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC101;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC101;


    @FXML private TableView<?> fetchStudentsTable;
    @FXML private TableView<Student> class1TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs101;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs101;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs101;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs101;

    private ObservableList<Student> studentList101 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();


    private final String CLASS_ID = "101";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId101.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId101.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId101.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass1() {
    ...
    private void loadClass1Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '101'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs101.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs101.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs101.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs101.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class1TableView.setItems(studentList101);
            takenByTeacherClass101.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC101.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC101.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC101.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC101.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId101.setText("Assigned Teacher: " + selectedUsername);
