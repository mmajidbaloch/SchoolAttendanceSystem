...
public class AdminClass7Controller {

    @FXML private Label teacherNameWithClassId107;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC107;
    @FXML private Button updateStudentAttendenceRecordC107;
    @FXML private TableColumn<Student, String> takenByTeacherClass107;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC107;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC107;


    // Table column references (optional hookup later if needed)
    @FXML private TableView<?> fetchStudentsTable; // Just a placeholder
    // Add other columns here...
    @FXML private TableView<Student> class7TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs107;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs107;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs107;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs107;

    private ObservableList<Student> studentList107 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();


    private final String CLASS_ID = "107";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId107.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId107.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId107.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass7() {
    ...
    private void loadClass7Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '107'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs107.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs107.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs107.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs107.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class7TableView.setItems(studentList107);
            takenByTeacherClass107.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC107.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC107.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC107.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC107.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId107.setText("Assigned Teacher: " + selectedUsername);
