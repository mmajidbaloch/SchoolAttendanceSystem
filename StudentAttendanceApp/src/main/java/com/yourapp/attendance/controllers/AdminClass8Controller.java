...
public class AdminClass8Controller {

    @FXML private Label teacherNameWithClassId108;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC108;
    @FXML private Button updateStudentAttendenceRecordC108;
    @FXML private TableColumn<Student, String> takenByTeacherClass108;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC108;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC108;

    // Table column references (optional hookup later if needed)
    @FXML private TableView<?> fetchStudentsTable; // Just a placeholder
    // Add other columns here...
    @FXML private TableView<Student> class8TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs108;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs108;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs108;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs108;

    private ObservableList<Student> studentList108 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();

    private final String CLASS_ID = "108";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId108.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId108.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId108.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass8() {
    ...
    private void loadClass8Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '108'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs108.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs108.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs108.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs108.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class8TableView.setItems(studentList108);
            takenByTeacherClass108.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC108.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC108.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC108.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC108.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId108.setText("Assigned Teacher: " + selectedUsername);
