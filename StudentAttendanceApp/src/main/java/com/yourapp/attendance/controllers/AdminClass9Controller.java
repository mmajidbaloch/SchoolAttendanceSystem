...
public class AdminClass9Controller {

    @FXML private Label teacherNameWithClassId109;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC109;
    @FXML private Button updateStudentAttendenceRecordC109;
    @FXML private TableColumn<Student, String> takenByTeacherClass109;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC109;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC109;

    // Table column references (optional hookup later if needed)
    @FXML private TableView<?> fetchStudentsTable; // Just a placeholder
    // Add other columns here...
    @FXML private TableView<Student> class9TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs109;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs109;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs109;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs109;

    private ObservableList<Student> studentList109 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();

    private final String CLASS_ID = "109";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId109.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId109.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId109.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass9() {
    ...
    private void loadClass9Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '109'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs109.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs109.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs109.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs109.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class9TableView.setItems(studentList109);
            takenByTeacherClass109.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC109.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC109.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC109.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC109.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId109.setText("Assigned Teacher: " + selectedUsername);
