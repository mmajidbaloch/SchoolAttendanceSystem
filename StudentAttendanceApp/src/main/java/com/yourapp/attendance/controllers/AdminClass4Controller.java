...
public class AdminClass4Controller {

    @FXML private Label teacherNameWithClassId104;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC104;
    @FXML private Button updateStudentAttendenceRecordC104;
    @FXML private TableColumn<Student, String> takenByTeacherClass104;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC104;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC104;


    @FXML private TableView<?> fetchStudentsTable;
    @FXML private TableView<Student> class4TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs104;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs104;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs104;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs104;

    private ObservableList<Student> studentList104 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();


    private final String CLASS_ID = "104";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId104.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId104.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId104.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass4() {
    ...
    private void loadClass4Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '104'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs104.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs104.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs104.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs104.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class4TableView.setItems(studentList104);
            takenByTeacherClass104.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC104.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC104.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC104.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC104.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId104.setText("Assigned Teacher: " + selectedUsername);
