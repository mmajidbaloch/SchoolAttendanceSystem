...
public class AdminClass3Controller {

    @FXML private Label teacherNameWithClassId103;
    @FXML private ChoiceBox<String> assignAttendenceTakerTeacherC103;
    @FXML private Button updateStudentAttendenceRecordC103;
    @FXML private TableColumn<Student, String> takenByTeacherClass103;
    @FXML private TableColumn<Student, Integer> calculateStudentPresentDaysC103;
    @FXML private TableColumn<Student, Integer> calculateStudentAbsentDaysC103;


    @FXML private TableView<?> fetchStudentsTable;
    @FXML private TableView<Student> class3TableView;
    @FXML private TableColumn<Student, Integer> fetchStudentIdClassIdIs103;
    @FXML private TableColumn<Student, String> fetchStudFNameClassIdIs103;
    @FXML private TableColumn<Student, String> fetchStudLNameClassIdIs103;
    @FXML private TableColumn<Student, String> fetchStudGenderClassIdIs103;

    private ObservableList<Student> studentList103 = FXCollections.observableArrayList();
    private Map<String, AttendanceInfo> attendanceMap = new HashMap<>();


    private final String CLASS_ID = "103";
    ...
    private void loadAssignedTeacher() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId103.setText("Assigned Teacher: " + username);
            } else {
                teacherNameWithClassId103.setText("Assigned Teacher: None");
            }

        } catch (SQLException e) {
            teacherNameWithClassId103.setText("Error loading teacher");
            e.printStackTrace();
        }
    }

    private void loadAttendanceDataForClass3() {
    ...
    private void loadClass6Students() {
        String query = "SELECT student_id, first_name, last_name, father_name, class_name, class_id, gender, enrolled_year, username, password FROM students WHERE class_id = '103'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            fetchStudentIdClassIdIs103.setCellValueFactory(new PropertyValueFactory<>("studentId"));
            fetchStudFNameClassIdIs103.setCellValueFactory(new PropertyValueFactory<>("firstName"));
            fetchStudLNameClassIdIs103.setCellValueFactory(new PropertyValueFactory<>("lastName"));
            fetchStudGenderClassIdIs103.setCellValueFactory(new PropertyValueFactory<>("gender"));

            class3TableView.setItems(studentList103);
            takenByTeacherClass103.setCellValueFactory(cellData ->
            ...
            calculateStudentPresentDaysC103.setCellValueFactory(cellData ->
                    new ReadOnlyObjectWrapper<>(attendanceMap.getOrDefault(cellData.getValue().getStudentId(), new AttendanceInfo(0, 0, "")).presentDays));

            calculateStudentAbsentDaysC103.setCellValueFactory(cellData ->
    ...
    private void loadTeacherOptions() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            assignAttendenceTakerTeacherC103.setItems(usernames);
    ...
    private void updateTeacherAssignment() {
        String selectedUsername = assignAttendenceTakerTeacherC103.getValue();
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs.next()) {
                ...
                teacherNameWithClassId103.setText("Assigned Teacher: " + selectedUsername);
